defmodule ScribblBackend.WordManager do
  ...
  def start_reveal_timer(room_id) do
    ...
    # Only start the reveal timer if hints are allowed
    if hints_allowed do
      room_reveal_timer_key = KeyManager.reveal_timer(room_id)

      # Get the current word to determine reveal timing based on word length
      case get_current_word(room_id) do
        {:ok, word} when is_binary(word) ->
          ...
          if word_length < 2 do
            ...
            # Calculate timer duration based on word length
            # Shorter words get longer timers between reveals
            timer_duration = trunc(30 / (word_length / 2))
            RedisHelper.setex(room_reveal_timer_key, timer_duration, "reveal_letter")
          end

        _ ->
          # Default to 15 seconds if word can't be determined
