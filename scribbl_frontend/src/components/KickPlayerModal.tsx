...
export default function KickPlayerModal({
  ...
  const kickVoteInfo = usePlayerStore((state) => state.kickVoteInfo);
  
  // Close the modal if a player is kicked
  useEffect(() => {
    if (kickVoteInfo?.kicked) {
      onClose();
    }
  }, [kickVoteInfo, onClose]);
        ...
        <div className="space-y-2 max-h-60 overflow-y-auto pr-2 mb-4">
          {otherPlayers.length > 0 ? (
            otherPlayers.map(([playerId, playerName]) => {
              // Check if there's an active vote for this player
              const isVoteActive = kickVoteInfo?.target_player_id === playerId;
              
              // Check if the current user has already voted for this player
              const hasVoted = isVoteActive && kickVoteInfo?.voters?.includes(userId);
                    ...
                    {isVoteActive && (
                      <div className="mr-3 text-sm text-white/80">
                        <span className="font-semibold text-indigo-300">
                          {kickVoteInfo.votes_count}/{kickVoteInfo.required_votes}
                        </span> votes
                      </div>
                    )}
